...
def submit_task(prompt, guidance_scale=5, num_inference_steps=30, 
    ...
    url = "https://api.wavespeed.ai/api/v2/wavespeed-ai/wan-2.1/t2v-480p-ultra-fast"
    headers = {
    ...
    if response.status_code == 200:
        ...
        print("API响应:", result)  # 打印API响应以便调试
        request_id = result.get("data", {}).get("id")  # 从data中获取id作为request_id
        if request_id:
...
def poll_result(request_id, max_attempts=10, sleep_time=20):
    """
    轮询获取任务结果
